...
class BuildMigrationTests(unittest.TestCase):
    def test_migrate_media_moves_files(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            new_path = os.path.join(media_dir, "photos-55", "100.jpg")
            new_thumb = os.path.join(media_dir, "photos-55", "thumb_100.jpg")
            self.assertTrue(os.path.exists(new_path))
            self.assertTrue(os.path.exists(new_thumb))
            self.assertEqual(db.updated[-1][1], "photos-55/100.jpg")

    def test_topic_dir_prefers_title_with_id_suffix(self):
        config = {
            "publish_dir": "site",
            "media_pages_dir": "media-pages",
            "publish_media_hashtags": True,
        ...
        self.assertEqual(b._topic_dir(123, "Photos!"), "photos-123")
